    ...
    try:
        if from_bytes:
            ...
            try:
                 ...
                 # Assuming PCM is loaded at a known rate, e.g., 22050, need to resample
                 # This requires the original sample rate of PCM, which is not passed here.
                 # A better approach would be to handle PCM loading more robustly earlier.
                 # For now, let's assume a common rate and resample if needed.
                 # This part needs refinement based on how PCM data is expected.
                 pcm_sr = 22050 # Example assumption
                 if pcm_sr != sr:
                     y = librosa.resample(y, orig_sr=pcm_sr, target_sr=sr)
                     print(f"Resampled PCM from {pcm_sr}Hz to {sr}Hz")
            except Exception as pcm_e:
                 print(f"PCM fallback loading failed: {pcm_e}")
